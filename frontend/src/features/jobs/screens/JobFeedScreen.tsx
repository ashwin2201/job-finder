...
import JobFeedDetailPanel from "@/features/jobs/components/JobFeedDetailPanel"
import JobFeedHeader from "@/features/jobs/components/JobFeedHeader"
import JobFeedList from "@/features/jobs/components/JobFeedList"
import JobFeedSidebar from "@/features/jobs/components/JobFeedSidebar"
...
const JobFeed = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-muted/60 px-4 py-8">
        <div className={`mx-auto flex min-h-screen max-w-7xl items-center justify-center rounded-[34px] ${appTheme.shell}`}>
  ...
  if (jobPostings.length === 0 && !hasActiveFilters) {
    return (
      <div className="min-h-screen bg-muted/60 px-4 py-8">
        <div className={`mx-auto flex min-h-screen max-w-7xl items-center justify-center rounded-[34px] ${appTheme.shell}`}>
          <div className="max-w-md space-y-4 text-center">
            ...
            <button onClick={() => void fetchAndSetJobPostings()} className={`rounded-full px-6 py-3 font-medium transition ${appTheme.button}`}>
              Refresh Jobs
            </button>
          </div>
        </div>
        ...
        <div className={`grid min-h-0 gap-5 p-5 sm:p-6 ${showJobDetail ? "xl:grid-cols-1" : "xl:grid-cols-[minmax(0,1.4fr)_320px]"}`}>
          <section className="flex min-h-0 flex-col gap-4">
            <div className="flex shrink-0 items-center justify-between px-1">
              <div>
                <p className={`text-xs font-semibold uppercase tracking-[0.24em] ${appTheme.muted}`}>Live feed</p>
                <h2 className={`mt-1 text-xl font-semibold ${appTheme.title}`}>Matching roles</h2>
              </div>
            ...
            {filteredJobs.length ? (
              ...
              <div className={`${appTheme.panel} rounded-[24px] p-8 text-center`}>
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-accent text-primary">
                  <Flame className="h-6 w-6" />
                </div>
                <h3 className={`mt-4 text-xl font-semibold ${appTheme.title}`}>No jobs match those filters</h3>
                <p className={`mt-2 text-sm ${appTheme.muted}`}>Try a different keyword or clear the company quick-pick.</p>
              </div>
            )}
          </section>
