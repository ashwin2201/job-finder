...
const createCountRecord = <T extends string>(
  options: readonly T[],
): Record<T, number> =>
  Object.fromEntries(options.map((option) => [option, 0])) as Record<T, number>

const normalizeSource = (job: Job) =>
...
export const hasVisaSupport = (job: Job) => normalizeSource(job).includes("visa")

export const deriveJobBadges = (job: Job) => {
  const badges = [deriveLanguage(job)] as string[]
  const workStyle = deriveWorkStyle(job)

  if (hasVisaSupport(job)) {
    badges.unshift("Visa Sponsorship")
  }
...
export const getTopCompanies = (jobs: Job[], limit = 6) =>
  Array.from(new Set(jobs.map((job) => job.company))).slice(0, limit)
