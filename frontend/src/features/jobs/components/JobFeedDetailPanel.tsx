...
const JobFeedDetailPanel = ({ job, badges }: JobFeedDetailPanelProps) => {
  if (!job) {
    return (
      <aside className={`${appTheme.panel} flex min-h-[480px] items-center justify-center p-8 w-100vh`}>
        <div className="space-y-3 text-center">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fff2ef] text-[#ef4444]">
            <Sparkles className="h-6 w-6" />
          </div>
          <h3 className={`text-xl font-semibold ${appTheme.title}`}>Pick a listing</h3>
          <p className={`text-sm ${appTheme.muted}`}>Select a job card to preview the summary and requirements.</p>
        </div>
      </aside>
    )
  }
  ...
  return (
    <aside className={`${appTheme.panel}`}>
      <div className="border-b border-[#efece5] p-5">
        <div className="flex items-center gap-4 rounded-[20px] border border-[#efe9dd] bg-white p-4">
          ...
          <div className="min-w-0">
            <h3 className={`truncate text-lg font-semibold ${appTheme.title}`}>{job.company}</h3>
            <p className={`text-sm ${appTheme.muted}`}>Selected company</p>
          </div>
        </div>
      </div>
      ...
      <div className="h-44 border-y border-[#f3ede4] bg-[linear-gradient(180deg,rgba(255,244,240,0.7),rgba(255,255,255,0.92)),radial-gradient(circle_at_top_right,rgba(239,68,68,0.18),transparent_36%),linear-gradient(135deg,#fff0eb,#ffe8df)] p-4">
        <div className="relative h-full overflow-hidden rounded-[22px] border border-white/80 bg-[linear-gradient(135deg,#ffd8ce,#fff5f0)]">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            ...
            <h4 className={`mt-2 text-2xl font-semibold ${appTheme.title}`}>{job.title}</h4>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6 p-5">
          ...
          {badges.map((badge, index) => (
            <span
              ...
              className={`rounded-full px-3 py-1 text-xs font-semibold ${
                index === 0 ? "bg-[#ef4444] text-white" : appTheme.accentSoft
              }`}
            >
              {badge}
            </span>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className={`text-xl font-semibold ${appTheme.accent}`}>Job Description</h4>
            <ChevronDown className="h-5 w-5 text-[#8e8a80]" />
          </div>
          <p className={`text-sm leading-7 ${appTheme.muted}`}>
        ...
        <div className="grid gap-3 rounded-[20px] bg-[#faf8f3] p-4">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm ${appTheme.title}`}>{job.company}</span>
          </div>
          <div className="flex items-center gap-3">
            <MapPin className="h-4 w-4 text-[#ef4444]" />
            <span className={`text-sm ${appTheme.title}`}>{job.location || "Japan"}</span>
          </div>
        </div>
