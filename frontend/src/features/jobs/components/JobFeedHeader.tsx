...
const JobFeedHeader = ({ searchQuery, onSearchChange }: JobFeedHeaderProps) => {
  return (
    <header className="flex flex-col gap-5 border-b border-[#eceae4] px-5 py-5 sm:px-7 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <p className={`text-xs font-semibold uppercase tracking-[0.26em] ${appTheme.muted}`}>Tailored default feed</p>
        <h1 className={`mt-1 text-3xl font-semibold ${appTheme.title}`}>Job Listings</h1>
      </div>

      <div className="flex items-center gap-3">
        <div className={`flex min-w-0 flex-1 items-center gap-3 rounded-2xl px-4 py-3 ${appTheme.search} sm:min-w-[300px]`}>
