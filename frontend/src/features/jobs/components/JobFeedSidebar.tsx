...
const FilterSection = <T extends string>({
  ...
  return (
    <details open className={`${appTheme.panel} overflow-hidden rounded-[24px]`}>
      <summary className={`flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold ${appTheme.title}`}>
        <span>{title}</span>
    ...
    <aside className={`${appTheme.sidebar} min-h-full p-5`}>
      ...
      <div className={`${appTheme.panel} mt-5 p-4`}>
        <div className="flex items-center justify-between">
          <h2 className={`text-lg font-semibold ${appTheme.title}`}>Top Companies</h2>
          <span className={`text-xs font-semibold uppercase tracking-[0.24em] ${appTheme.muted}`}>Quick picks</span>
        </div>
      ...
      <button
        ...
        className={`mt-6 w-full rounded-[18px] px-4 py-4 text-sm font-semibold transition ${
          hasActiveFilters ? appTheme.button : "bg-muted text-muted-foreground"
        }`}
      >
        Clear Filters
      </button>
