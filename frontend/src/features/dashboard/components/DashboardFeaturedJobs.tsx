  ...
  return (
    <section>
      <div className="flex items-center justify-between px-1">
        <div>
          <p className={`text-xs font-semibold uppercase tracking-[0.24em] ${appTheme.muted}`}>Curated picks</p>
          <h2 className={`mt-1 text-2xl font-semibold ${appTheme.title}`}>Featured jobs</h2>
        </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3">
        {featuredJobs.map(({ title, company, salary, tags, palette, badgeIcon: Icon }) => (
          <article key={title} className={`${appTheme.panel} overflow-hidden`}>
            ...
            <div className="space-y-4 p-5">
              <p className={`text-lg font-semibold ${appTheme.title}`}>{salary}</p>
